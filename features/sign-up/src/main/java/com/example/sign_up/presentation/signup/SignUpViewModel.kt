...
class SignUpViewModel @AssistedInject constructor(
    ...
    fun signUp(signUpData: SignUpData) = debounce {
        viewModelScope.launch {
            try {
                ...
                commonUi.toast(resources.getString(R.string.sign_up_success))
                router.goBack()
    ...
    private fun handlePasswordMismatchException() {
        ...
        setFieldError(SignUpField.REPEAT_PASSWORD, resources.getString(R.string.password_mismatch))
    }

    private fun handleAccountAlreadyExistException() {
        focusField(SignUpField.EMAIL)
        setFieldError(SignUpField.EMAIL, resources.getString(R.string.account_already_exists))
    }
